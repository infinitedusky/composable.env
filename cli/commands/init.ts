...
function scaffoldDocker(cwd: string, envDir: string): void {
  ...
  if (!fs.existsSync(architectureDoc)) {
    fs.writeFileSync(architectureDoc,
      ...
      '`ce build` generates `.env` files and `docker-compose.yml` from these sources.\n\n' +
      '## Docker\n\n' +
      'All services run in Docker via `docker compose`. The compose file is fully generated by `ce build` — never hand-edit it.\n\n' +
      '```bash\n' +
      'ce build                          # generate all outputs\n' +
      'docker compose --profile local up # start local environment\n' +
      '```\n'
    );
  ...
  if (fs.existsSync(gitignorePath)) {
    ...
    if (!content.includes('docker-compose.yml')) {
      ...
      fs.writeFileSync(gitignorePath, content);
      console.log(chalk.green('  updated .gitignore — added docker-compose.yml'));
    }
  }
