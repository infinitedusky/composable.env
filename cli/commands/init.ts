...
function scaffoldDocker(cwd: string, envDir: string, syncOnly: boolean = false): void {
  ...
  // Wholesale workspace COPY: package.json + apps/ + packages/ all go
  // into the install layer. Trades cache-correctness (any package change
  // invalidates the install layer for every app) for compatibility with
  // pnpm's dependenciesMeta.injected workspace deps, which `turbo prune
  // --docker` cannot serialize. If a project doesn't use injected deps
  // and wants better cache behavior, replace with the prune pattern by
  // hand. Source code arrives via volume mount in dev.
  ...
  if (existingDev !== dockerfileDevContent) {
    ...
    console.log(chalk.green(`  ${verb} docker/Dockerfile.nextdev`));
  }
  ...
  // Same wholesale workspace COPY pattern as Dockerfile.nextdev.
  ...
  const vitepressDevContent =
    ...
    '\n' +
    '# Workspace install: COPY apps/ + packages/ so pnpm can resolve every\n' +
    '# workspace dep, including pnpm dependenciesMeta.injected entries.\n' +
    'COPY package.json pnpm-lock.yaml pnpm-workspace.yaml ./\n' +
    'COPY apps/ ./apps/\n' +
    'COPY packages/ ./packages/\n' +
    'RUN pnpm install --frozen-lockfile\n' +
