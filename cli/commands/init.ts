...
/**
 * Scan env/contracts/*.contract.json for apps (location starts with "apps/").
 * Returns sorted list of app directory names (e.g., ["admin", "poker"]).
 * Empty if no contracts exist (e.g., during initial scaffold).
 */
function scanAppContracts(cwd: string, envDir: string): string[] {
  const contractsDir = path.join(cwd, envDir, 'contracts');
  ...
  for (const file of fs.readdirSync(contractsDir)) {
    ...
    try {
      ...
      if (contract.location?.startsWith('apps/')) {
        const appName = contract.location.slice('apps/'.length).split('/')[0];
...
/**
 * Generate per-app COPY lines for the install layer.
 * Returns empty string if no apps found (initial scaffold).
 * The placeholder COPY apps/example-app/package.json line is included
 * for new projects so the Dockerfile is valid even before contracts exist.
 */
function generateAppPackageCopies(apps: string[]): string {
  if (apps.length === 0) {
    return '# Per-app package.json files (auto-enumerated from contracts on scaffold:sync)\n' +
           '# COPY apps/<name>/package.json apps/<name>/\n';
  }
  const header = '# Per-app package.json files (auto-enumerated from contracts on scaffold:sync)\n';
  const lines = apps.map(app => `COPY apps/${app}/package.json apps/${app}/`).join('\n');
  return header + lines + '\n';
}
...
function scaffoldDocker(cwd: string, envDir: string, syncOnly: boolean = false): void {
  ...
  // Always (re)written from the contract list — this is generated infrastructure,
  // not user code. Per-app COPY lines are needed so pnpm can resolve workspace deps.
  const dockerfileDevPath = path.join(dockerDir, 'Dockerfile.nextdev');
  const apps = scanAppContracts(cwd, envDir);
  const appCopies = generateAppPackageCopies(apps);
  const dockerfileDevContent =
    '# Next.js local development — hot reload via volume mounts\n' +
    '# GENERATED by composable.env (ce init / scaffold:sync). Per-app COPY lines\n' +
    '# below are auto-enumerated from env/contracts/*.contract.json. Re-run\n' +
    '# scaffold:sync after adding a new app contract.\n' +
    'FROM node:20-alpine\n' +
    '\n' +
    ...
    'WORKDIR /app\n' +
    '\n' +
    '# Workspace manifests (cached layer — only invalidated when manifests change)\n' +
  ...
  if (existingDev !== dockerfileDevContent) {
    ...
    const appsSummary = apps.length > 0 ? ` (${apps.length} apps)` : ' (no apps yet)';
    console.log(chalk.green(`  ${verb} docker/Dockerfile.nextdev${appsSummary}`));
  }
  ...
  // Always (re)written from the contract list for the same reason as Dockerfile.nextdev.
  ...
  if (existingVitepress !== vitepressDevContent) {
    ...
    console.log(chalk.green(`  ${verb} docker/Dockerfile.vitepressdev`));
  }
